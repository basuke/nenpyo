// Issue #6 — 末尾の括弧を tagline（キャッチコピー）と事実補足に振り分ける提案を作る。
//
...
//
// 判定の土台は #7 で裏を取った「括弧が末尾にあるものだけがキャッチコピーでありうる」。
// そのうえで、末尾の括弧の中をさらに事実とキャッチコピーに分ける。
...
const OVERRIDE = {
  ...
  "日本初のマイコン専門誌": "tagline",
  "72秒間の未解決": "tagline",
  // タイトルのほうがキャッチコピーで、括弧が事実という逆転
  "車載AIアシスタント": "fact",
  "ゼロショット音声合成": "fact",
  // 規則がキャッチコピーと見たが、実際は事実の補足
  "音楽はキース・エマーソン": "fact",
  "表紙は川原由美子": "fact",
  "原作は2009年刊行": "fact",
  "未来編を映像化": "fact",
  ...
  "映像も破られた": "tagline",
  "コードも操作も委任": "tagline",
  // #5 で子タイトルへ残した事実。助詞を含むので規則はキャッチコピーと見てしまう
  "ファミコンと同日": "fact",
  // 固有名詞を「と」「は」で並べただけのもの。助詞があるので規則はキャッチコピーと見るが、
  // 中身は誰が・何が、の事実。タイトルの側がキャッチコピーになっている逆転も多い。
  "MPManとRio": "fact",
...
for (const event of events) {
  ...
  for (const title of titles) {
    ...
    // 事実だけが残るならタイトルは元のまま。キャッチコピーが混ざるときだけ組み替える。
    ...
    //   事実とキャッチコピーが混在  … 「。」で割った結果。切り方が妥当か見る値打ちがある
    const low =
...
const doc = {
  $comment: [
    "Issue #6 — 末尾の括弧を tagline（キャッチコピー）と事実補足に振り分けた表。",
    ...
    "tagline  … 切り出したキャッチコピー（notes.tagline に入る）。無ければ null",
